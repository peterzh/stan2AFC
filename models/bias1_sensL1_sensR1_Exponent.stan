...
generated quantities {
	...
	for (c in 1:numTestContrasts) {
		...
		{
			zTest[c,sess] = bias + bias_delta_perSession[sess] + (sens_left + sens_left_delta_perSession[sess])*testContrastLeft[c]^n_exp + (sens_right + sens_right_delta_perSession[sess])*testContrastRight[c]^n_exp ;
